...
switch opt.alg
    ...
    case 'SpectralGap'
        ...
        if (0 ==1)
            m=200;
            figure;subplot(231);            
            plot(s(1:m), 'k:*'); hold on; 
            subplot(232);plot(g(1:m), 'r:*'); hold on; 
            subplot(233);plot(relative_g(1:m), 'b-^'); hold on;
            subplot(234);plot(weight_rel_g(1:m), 'g-+'); hold on;
            subplot(235);plot(nml_weight_rel_g(1:m), 'k-s');            
            subplot(236);plot(opt_match_1(1:m), 'm-o');
        end            
