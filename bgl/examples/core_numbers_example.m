...
gplot(A, xy);
hold on;
plot(xy(:, 1), xy(:, 2), 'r.', 'MarkerSize', 24);
hold off;
text(xy(:, 1)+0.1, xy(:, 2)+0.1, num2str((1:21)'));
set(gcf, 'Color', [1, 1, 1]);
set(gca, 'XTick', []);
set(gca, 'YTick', []);
xlim([-1, 10]);
...
for c = unique(cn')
    ...
    lw = 16 * 1.5^(max(cn) - c);
    xym = xy(m, :);
    is = convhull(xym(:, 1), xym(:, 2));
    h = fill(xym(is, 1), xym(is, 2), cl);
    set(h, 'LineWidth', lw, 'EdgeColor', cl);
end
gplot(A, xy, 'k-');
plot(xy(:, 1), xy(:, 2), 'r.', 'MarkerSize', 24);
text(xy(:, 1)+0.1, xy(:, 2)+0.1, num2str((1:21)'));
...
A = spones(A); % convert to unweighted
[cn, csz] = core_numbers(A);
cs = unique(cn);
